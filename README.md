...
Signaling, ICE exchange, camera/mic media, and reconnection/stall handling are
all in place.
...
- **Later, if warranted:** TURN (only if a real connection ever fails), a
  YouTube adapter (embeddable IFrame API), Netflix/Prime via a browser extension
  (the Teleparty model).
