    ...
    <main className='mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-4 p-4 sm:p-8'>
        ...
        <div className='flex items-center gap-2 text-sm text-slate-600'>
          <span
            className={`size-2.5 rounded-full ${
              view.status === 'connected' ? 'bg-emerald-500' : 'animate-pulse bg-amber-400'
            }`}
          />
          {view.status === 'connected' ? 'Connected' : 'Connecting'}
        </div>
      </header>
      ...
      <form className='flex gap-2' onSubmit={handleSubmit}>
        <input
          ...
          placeholder={view.status === 'connected' ? 'Write a message' : 'Waiting for peer…'}
          value={draft}
        />
        <button
