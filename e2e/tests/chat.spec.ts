...
test('unread indicator shows for messages received while the chat is closed', async ({ room }) => {
  ...
  await expect(host.getByRole('list', { name: 'Chat messages' }).getByText(message)).toBeVisible();
